...
Item {
    ...
    property bool _showDigitPad: (!useMaze && showDigitPad) || emergency
    ...
    Label {
        ...
        anchors {
            ...
            bottom: showEmergencyButton || keypad.visible || useMaze ? keypad.top : alphanumProxy.bottom
    ...
    Connections {
        ...
        onFeedback: {
            var attempt = root.authenticator.maximumAttempts - attemptsRemaining + 1
    ...
    Item {
        ...
        MouseArea {
            ...
            Canvas {
                ...
                onPaint: {
                    ...
                    if (root.enteredPin.length > 0) {
                        ...
                        if (pinArea.pressed && pinArea.lastx > 0 && pinArea.lasty > 0) {
                            ctx.strokeStyle = Theme.rgba(mazeLockSettings.colored ? mazeLock.colors[i] : Theme.primaryColor, 0.5)
                            ctx.lineTo(pinArea.lastx, pinArea.lasty)
                            ctx.stroke()
                        }
                    }
                }
            ...
            onReleased: {
                ...
                } else if (root.enteredPin.length > 4) {
                    root._clickedConfirmButton()
                }
            }
        }
